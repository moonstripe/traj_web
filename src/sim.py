"""The numerical simulation. Basic text interface provided when run as main."""

from math import log, pi, sin, cos, exp, sqrt
import pandas as pd
global VBO
global calcrange
...
class Simulation(object):
    ...
    def __init__(self):
        # stage parameter lists typing
        # Kojin note: replaced default string values with specific typing so pylance stops yelling at me :(
        self.numstages: int = 0
        self.burntime: list[float] = []
        self.thrust0: list[float] = []
        self.Isp0: list[float] = []
        self.m0: list[float] = []
        self.fuelfraction: list[float] = []
        self.fuelmass: list[float] = []
        self.dMdt:list[float] = []

        # non dependent on number of stages
        self.payload: float = 0.0
        self.missilediam: float = 0.0
        self.nozzlearea: float = 0.0
        self.LdivD: float = 0.0
        self.Nosecone: str = ''
        self.rvdiam: float = 0.0
        self.est_range: float = 0.0
        self.burnout_angle: float = 0.0

        #results dict
        self.data: dict[str, list[float]] = {'Time':[0],'Height':[0],'Mass':[0],'Velocity':[0],'Thrust':[0],'Drag':[0],'CD':[0],'Gamma':[pi/2],'Range':[0]}
    ...
    def integrate(self,trajectory):
        ...
        if self.numstages == 1:
           VBO = self.Isp0[0]*9.81*log(mtot/(mtot - self.fuelmass[0]))
        #    print(VBO)
        if self.numstages ==2:
           VBO = self.Isp0[0]*9.81*log(mtot/(mtot - self.fuelmass[1]))
           VBO = VBO + self.Isp0[1]*9.81*log((mtot-self.m0[0])/(mtot - self.m0[0] - self.fuelmass[1]))
        #    print(VBO)
        if self.numstages ==3:
           VBO = 0.
           VBO = self.Isp0[0]*9.81*log(mtot/(mtot - self.fuelmass[0]))
           VBO = VBO + self.Isp0[1]*9.81*log((mtot-self.m0[0])/(mtot - self.m0[0] - self.fuelmass[1]))
           VBO = VBO + self.Isp0[2]*9.81*log((mtot-self.m0[0]-self.m0[1])/(mtot -self.m0[0] -self.m0[1] - self.fuelmass[2]))
        #    print(VBO)
        if self.numstages ==4:
           VBO = 0.
           VBO = self.Isp0[0]*9.81*log(mtot/(mtot - self.fuelmass[0]))
           VBO = VBO + self.Isp0[1]*9.81*log((mtot-self.m0[0])/(mtot - self.m0[0] - self.fuelmass[1]))
           VBO = VBO + self.Isp0[2]*9.81*log((mtot-self.m0[0]-self.m0[1])/(mtot -self.m0[0] -self.m0[1] - self.fuelmass[2]))
           VBO = VBO + self.Isp0[3]*9.81*log((mtot-self.m0[0]-self.m0[1]-self.m0[2])/(mtot -self.m0[0] -self.m0[1] - self.m0[2]-self.fuelmass[3]))
        #    print(VBO)
        if self.numstages ==5:
           VBO = 0.
           VBO = self.Isp0[0]*9.81*log(mtot/(mtot - self.fuelmass[0]))
           ...
           VBO = VBO + self.Isp0[2]*9.81*log((mtot-self.m0[0]-self.m0[1])/(mtot -self.m0[0] -self.m0[1] - self.fuelmass[2]))
           VBO = VBO + self.Isp0[3]*9.81*log((mtot-self.m0[0]-self.m0[1]-self.m0[2])/(mtot -self.m0[0] -self.m0[1] - self.m0[2]-self.fuelmass[3]))
           VBO = VBO + self.Isp0[4]*9.81*log((mtot-self.m0[0]-self.m0[1]-self.m0[2]-self.m0[3])/(mtot -self.m0[0] -self.m0[1] - self.m0[2]-self.m0[3]-self.fuelmass[4]))
        #    print('VBO=',VBO/1000.)
        ...
        while t < tEND and h > 0: # big loop
            ...
            if (t + deltat/5) > burntimetot:
               ...
               Thrust = self.Isp0[0]*self.dMdt[0]*9.81 + nozarea*pi*(p0-p_height)
            elif nstage > 1:
               Thrust = self.Isp0[nstage-1]*self.dMdt[nstage-1]*9.81
            ...
            if (t + deltat / 5) > tlimit and flag == True:
                ...
                if nstage == 2:
                    m = mtot - self.m0[0] - self.m0[1]
                    # print('NSTAGE=',nstage, m )                   
                if nstage == 3:
                    m = mtot - self.m0[0] - self.m0[1] - self.m0[2]
                    # print('NSTAGE=',nstage, m  )  
                if nstage == 4:
                    m = mtot - self.m0[0] - self.m0[1] - self.m0[2] - self.m0[3]        
                m_old = m
...
if __name__ == "__main__":
    ...
    print("")
    sim = Simulation() #this simulation object has no parent
    sim.numstages = int(input("Number of stages: "))
    for i in range(0,sim.numstages):
    ...
    sim.Nosecone = str(input("Nosecone: "))
    sim.LdivD = float(input("Length/Diam (dimensionless): ")) 
    ...
    headers = ['Time', 'Height', 'Mass', 'Velocity', 'Thrust', 'Drag', 'CD', 'Gamma', 'Range']
